...
`status.json` is not in this repo. It is written on the server every 2 minutes from `rnstatus -j` and systemd, and the page reads it from its own origin (scotmesh.net reads it cross-origin, so keep CORS open on it). `status.html` is the embeddable status strip built from the same file.
